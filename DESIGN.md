...
PhotoListView.task
   └─ .onAppear ──▶ PhotoListStore.send
        └─ NetworkFirstPhotoRepository.loadNext
             ...
             └─ 실패 ─▶ MetadataStore.fetchShuffled  ─▶ .loaded(source: .cache(reason:))
                          └─ 저장분도 비었으면
                               ├─ 처음부터 아무것도 없음 ─▶ .failed(AppError)
                               └─ 이어서 불러오다 떨어짐 ─▶ .loaded(빈 묶음) = 더 없음
```

### 실패는 원인을 잃지 않고 화면까지 갑니다

대체했다는 사실만 전하면 화면은 원인을 짐작하게 됩니다. 처음에는 그렇게 만들었고, **호출 제한(429)에 걸렸는데도 "연결이 없어서"라고 안내**했습니다. 저장분이 있는 한 `AppError.rateLimited` 는 화면에 닿지 못했습니다 — 즉 대기 시간을 파싱해 둔 코드가 첫 실행에서만 쓰였습니다.

그래서 `PhotoSource.cache` 가 원인을 함께 나릅니다.

| 상황 | `source` | 화면 문구 |
|---|---|---|
| 정상 | `.network` | 없음 |
| 오프라인 + 저장분 있음 | `.cache(reason: .offline)` | 연결이 없어 저장된 이미지를 보여주고 있어요 |
| 429 + 저장분 있음 | `.cache(reason: .rateLimited(30))` | 요청이 많아 저장된 이미지를 보여주고 있어요. 30초 뒤에… |
| 오프라인 + 저장분 없음 | — (`.failed`) | 연결이 없고 저장된 이미지도 없어요 |

같은 실패라도 **화면에 보여줄 것이 있는지**에 따라 할 말이 달라지므로, 문구를 `message`(아무것도 없을 때)와 `fallbackMessage`(대체해서 보여줄 때) 둘로 나눴습니다. 이것은 실패의 성질이 아니라 화면의 사정이라 앱 타깃에 둡니다.

### 끝은 응답이 아니라 결과로 판단합니다

이 API 에는 "마지막 페이지" 신호가 없습니다. 매번 무작위 묶음이 오고 이미 본 것만 담겨 오기도 합니다. 그러면 목록이 늘지 않고, 화면의 마지막 줄도 그대로라 다음 요청을 스스로 부르지 못한 채 멈춥니다 — 사용자에게는 스크롤 끝에서 아무 반응이 없는 상태입니다.

`PhotoListStore` 가 새 항목이 나올 때까지 **최대 3회** 다시 요청하고, 그래도 없으면 `hasMore` 를 내려 더 요청하지 않습니다. 목록 끝에는 "더 불러올 이미지가 없어요"를 붙여, 멈춘 것과 끝난 것을 사용자가 구분할 수 있게 했습니다.

**3회의 근거**: 실제로 20번 호출해 재 보니 받은 200개 중 고유한 것이 198개로, **중복률은 1%** 였습니다. 한 묶음 열 개가 전부 중복이어야 다시 시도하므로, 이 경로는 사실상 발동하지 않습니다. 무한 스크롤이 멈추는 것을 막는 안전장치이지 상시 동작하는 길이 아닙니다.

**대가**: 그래도 정말로 끝인 경우에는 소득 없는 요청이 2회 더 나갑니다. 끝에 머무는 동안 무한히 반복되는 것보다는 낫다고 봤습니다.
